...
namespace NHOM1_QLKHO.DAO
{
    class CommodityDAO { 
        ...
        public List<Commodity> GetAll()
        {
                ...
                DataTable data = DataProvider.Instance.ExecuteQuery("Commodity_GetAll");
        ...
        {
            List<Commodity> list = new List<Commodity>();
            DataTable data = DataProvider.Instance.ExecuteQuery("GetCommodityList");
            foreach (DataRow item in data.Rows)
            {
                Commodity entry = new Commodity(item);
                list.Add(entry);
        ...
        public List<Commodity> GetCommodityListByName(string commodityname)
        {
            List<Commodity> list = new List<Commodity>();
            DataTable data = DataProvider.Instance.ExecuteQuery("SearchCommodityByName @commidityname", new object[] { commodityname });
            foreach (DataRow item in data.Rows)
            {
                Commodity entry = new Commodity(item);
                list.Add(entry);
